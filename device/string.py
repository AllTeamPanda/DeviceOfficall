...
def encode():
     ...
     if len(ppk) in _PYRO_FORM.keys():
         ...
         encodestring = base64.urlsafe_b64encode(struct.pack(_STRUCT_PREFORMAT.format(4), dc_id, ipaddress.ip_address(DC_IPV4[dc_id]).packed, 443, auth_key,)).decode("ascii")
         if len(encodestring):
